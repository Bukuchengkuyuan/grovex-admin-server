...
public class JWTConfig {
    ...
    // 定义超时时间 为3天
    private final long EXPIRE = 3 * 24 * 60 * 60;
            ...
            if (JwtUtil.checkToken(token)) {
                ...
                if (StringUtils.isNotEmpty(redisToken)) {
                    ...
                    if (redisToken.equals(token)) {
                        ...
                        redisUtil.set(String.valueOf(redisJwt.getId()), token, 60 * 60 * 3);
                        return true;
                    }
                }
            }
    ...
    public String refreshToken(String token, HttpServletResponse response) {
        ...
        // 若当前 token 过期时间 小于 12h，则 更新 token
        if ((expire - currentTime) / 1000 < 5 * 60 * 12) {
            // 获取 token 数据
            JwtVo jwt = GsonUtil.fromJson(String.valueOf(claims.get("data")), JwtVo.class);
            // 重新生成 token
